...
:global vWN; :global vWS; :global vWP; :global vWSec; :global vWCh; :global vWDrv
:global vCI; :global vCM; :global vCG; :global vCR; :global vCT
:set vWN [:toarray ""]; :set vWS [:toarray ""]; :set vWP [:toarray ""]; :set vWSec [:toarray ""]; :set vWCh [:toarray ""]; :set vWDrv [:toarray ""]
:set vCI [:toarray ""]; :set vCM [:toarray ""]; :set vCG [:toarray ""]; :set vCR [:toarray ""]; :set vCT [:toarray ""]

:local axSrc ":global vWN; :global vWS; :global vWP; :global vWSec; :global vWCh; :global vWDrv; :global vCI; :global vCM; :global vCG; :global vCR; :global vCT; :foreach w in=[/interface/wifi find] do={ :local nm [/interface/wifi get \$w name]; :local ss \"\"; :do { :set ss [/interface/wifi get \$w configuration.ssid] } on-error={}; :if ([:len \$ss]=0) do={ :do { :set ss [/interface/wifi get \$w ssid] } on-error={} }; :local pk \"\"; :do { :set pk [/interface/wifi get \$w security.passphrase] } on-error={}; :local sc \"\"; :do { :set sc [:tostr [/interface/wifi get \$w security.authentication-types]] } on-error={}; :local ch \"\"; :do { :local mm [/interface/wifi monitor \$nm once as-value]; :set ch [:tostr (\$mm->\"channel\")] } on-error={}; :set vWN (\$vWN,\$nm); :set vWS (\$vWS,\$ss); :set vWP (\$vWP,\$pk); :set vWSec (\$vWSec,\$sc); :set vWCh (\$vWCh,\$ch); :set vWDrv (\$vWDrv,\"ax\") }; :foreach r in=[/interface/wifi/registration-table find] do={ :local sg \"\"; :do { :set sg [/interface/wifi/registration-table get \$r signal] } on-error={}; :set vCI (\$vCI,[/interface/wifi/registration-table get \$r interface]); :set vCM (\$vCM,[/interface/wifi/registration-table get \$r mac-address]); :set vCG (\$vCG,\$sg); :set vCR (\$vCR,\"\"); :set vCT (\$vCT,\"\") }"
:local acSrc ":global vWN; :global vWS; :global vWP; :global vWSec; :global vWCh; :global vWDrv; :global vCI; :global vCM; :global vCG; :global vCR; :global vCT; :foreach w in=[/interface/wireless find] do={ :local nm [/interface/wireless get \$w name]; :local ss \"\"; :do { :set ss [/interface/wireless get \$w ssid] } on-error={}; :local sp \"\"; :do { :set sp [/interface/wireless get \$w security-profile] } on-error={}; :local pk \"\"; :local sc \"\"; :do { :local sid [/interface/wireless/security-profiles find name=\$sp]; :if ([:len \$sid]>0) do={ :set pk [/interface/wireless/security-profiles get \$sid wpa2-pre-shared-key]; :set sc [:tostr [/interface/wireless/security-profiles get \$sid authentication-types]] } } on-error={}; :local ch \"\"; :do { :local mm [/interface/wireless monitor \$nm once as-value]; :set ch [:tostr (\$mm->\"channel\")] } on-error={}; :set vWN (\$vWN,\$nm); :set vWS (\$vWS,\$ss); :set vWP (\$vWP,\$pk); :set vWSec (\$vWSec,\$sc); :set vWCh (\$vWCh,\$ch); :set vWDrv (\$vWDrv,\"ac\") }; :foreach r in=[/interface/wireless/registration-table find] do={ :local sg \"\"; :do { :set sg [/interface/wireless/registration-table get \$r signal-strength] } on-error={}; :local rr \"\"; :do { :set rr [/interface/wireless/registration-table get \$r rx-rate] } on-error={}; :local tt \"\"; :do { :set tt [/interface/wireless/registration-table get \$r tx-rate] } on-error={}; :set vCI (\$vCI,[/interface/wireless/registration-table get \$r interface]); :set vCM (\$vCM,[/interface/wireless/registration-table get \$r mac-address]); :set vCG (\$vCG,\$sg); :set vCR (\$vCR,\$rr); :set vCT (\$vCT,\$tt) }"
...
:local iW 0
:while ($iW < [:len $vWN]) do={
    :local o ("{\"interface\":\"" . ($vWN->$iW) . "\",\"driver\":\"" . ($vWDrv->$iW) . "\",\"ssid\":\"" . [$vigilantClean ($vWS->$iW)] . "\",\"passphrase\":\"" . [$vigilantClean ($vWP->$iW)] . "\",\"security\":\"" . [$vigilantClean ($vWSec->$iW)] . "\",\"channel\":\"" . [$vigilantClean ($vWCh->$iW)] . "\"}")
